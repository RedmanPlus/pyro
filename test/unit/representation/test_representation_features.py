...
def test_add_labels_to_rep(snapshot):
    ...
    command_a = Command(
        operation=CommandType.STORE, target=Variable(name="a"), operand_a="5"
    )
    command_b = Command(
        operation=CommandType.STORE, target=Variable(name="b"), operand_a="6"
    )
    command_c = Command(
        operation=CommandType.STORE, target=Variable(name="c"), operand_a="7"
    )
    command_d = Command(
        operation=CommandType.STORE, target=Variable(name="d"), operand_a="8"
    )
...
def test_cmp_and_jump_int_rep(snapshot):
    ...
    command_e = Command(
        operation=CommandType.STORE, target=Variable(name="e"), operand_a="9"
    )
    representation.append(command_a)
    representation.append(command_b)
    representation.append(command_c)
    ...
    command_jne = Command(
        operation=CommandType.JNE, operand_a=Label(name="foo"), target=None
    )
    command_jmp = Command(
        operation=CommandType.JMP, operand_a=Label(name="bar"), target=None
    )
...
def test_struct_declaration_int_rep(snapshot):
    ...
    rep.add_definition("Foo", fields={"a": 0})
    ...
    command_declare_foo = Command(
        ...
        target=Variable(
            name="foo", var_type=rep.get_declaration_by_name("Foo")
        ),
        operand_a=StructDeclaration(
            rep.get_declaration_by_name("Foo"), field_values=["1"]
        ),
    )
    ...
    command_declare_bar_a = Command(
        operation=CommandType.STORE,
        target=Variable(
            name="bar_a", var_type=rep.get_declaration_by_name("Bar")
        ),
        operand_a=StructDeclaration(
            rep.get_declaration_by_name("Bar"), field_values=["1", "2"]
        ),
    )
    ...
    command_declare_bar_b = Command(
        operation=CommandType.STORE,
        target=Variable(
            name="bar_b", var_type=rep.get_declaration_by_name("Bar")
        ),
        operand_a=StructDeclaration(
            rep.get_declaration_by_name("Bar"), field_values=["1", "2"]
        ),
    ...
    command_declare_baz = Command(
        operation=CommandType.STORE,
        target=Variable(
            name="baz", var_type=rep.get_declaration_by_name("Baz")
        ),
        operand_a=StructDeclaration(
    ...
    snapshot.assert_match(
        rep.pprint(), "representation_struct_declaration_usage"
    )
